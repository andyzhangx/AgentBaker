...
Describe 'cse_install_mariner.sh'
    ...
    Describe 'installAznfsPackage'
        ...
        It 'installs pre-downloaded RPM on AzureLinux 3.0'
            OS_VERSION="3.0"
            # Override find to return our test RPM
            find() {
        ...
        It 'fails when pre-downloaded RPM is not found'
            OS_VERSION="3.0"
            # Override find to return empty (no RPM found)
            find() {
                echo ""
            }
            When run installAznfsPackage
            The output should include "aznfs RPM not found"
            The status should equal "$ERR_AZNFS_INSTALL_FAIL"
        End
    End
End
